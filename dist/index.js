...
async function until(cb, interval, maxRetries = Infinity, timeoutMessage) {
  ...
  throw new Error(timeoutMessage != null ? timeoutMessage : "Maximun retry count reached");
}
...
// src/types/generated/Example.ts
init_cjs_shims();
...
function validate(item, schema, options = {}) {
  ...
  if (options.message) {
    return import_joi.default.attempt(item, schema, `${options.message}:`, joiOpts);
  }
  return import_joi.default.attempt(item, schema, joiOpts);
}
...
var Logger = class {
  ...
  fmt(item) {
    return (0, import_util.inspect)(item, {
      showHidden: false,
      depth: null,
      maxArrayLength: null,
      sorted: true
...
function ensureDefined(input, message) {
  if (input === void 0 || input === null) {
    throw new Error(message != null ? message : `Expected input to be defined, got ${String(input)}`);
  }
  return input;
}
