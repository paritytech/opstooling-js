...
declare const getShellCommandRunner: (parentLogger: Logger, initialConfiguration: ShellCommandRunnerConfiguration) => ShellCommandRunner;
...
declare function until(cb: () => MaybePromise<boolean>, interval: number, maxRetries?: number, timeoutMessage?: string): Promise<void>;

declare type FreeId = {
    id: string;
    release: () => void;
};
declare const getFreeId: () => FreeId;
declare function ensureDefined<T>(input: T | undefined | null, message?: string): NonNullable<T>;

declare type Options = {
    /** Custom error message, will be followed by validation errors from Joi */
    message?: string;
};
declare function validate<T>(item: unknown, schema: Joi.Schema, options?: Options): T;

export { Err, Example, ExtendedOctokit, FreeId, JSONValue, Logger, LoggerOptions, LoggingImplementation, LoggingLevel, LoggingLevels, MaybePromise, Ok, ToOptional, ToRequired, delay, displayError, displayShellCommand, doValuesContainSameData, ensureDefined, envNumberVar, envVar, err, getFreeId, getOctokit, getShellCommandRunner, intoError, isGithubOrganizationMember, ld, normalizeValue, ok, redact, until, validate, validatedFetch };
